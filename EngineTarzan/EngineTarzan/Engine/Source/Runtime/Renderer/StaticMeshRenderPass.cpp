...
#pragma endregion W08
    ...
    {
        // 컴포넌트 및 스켈레탈 메시 유효성 검사
        if (!Comp || !Comp->GetSkeletalMesh() || !Comp->GetSkeletalMesh()->GetRenderData())
        {
            continue;
        }
        // 액터 숨김 여부 등 추가적인 컬링 조건 확인 가능
        if (Comp->GetOwner() && Comp->GetOwner()->IsHidden())
        {
            continue;
        }
        if (Comp->GetWorld() != GEngine->ActiveWorld) // 현재 활성 월드에 속하는지 확인
        {
            continue;
        }
        ...
        // 오브젝트 상수 버퍼 업데이트 (스켈레탈 메시용)
        FMatrix WorldMatrix = Comp->GetWorldMatrix(); // 스켈레탈 메시 컴포넌트의 월드 매트릭스 사용
        FVector4 UUIDColor = Comp->EncodeUUID() / 255.0f; // 스켈레탈 메시 컴포넌트의 UUID 사용

        // 선택 여부 확인 (EditorEngine 사용 방식 유지)
        UEditorEngine* Engine = Cast<UEditorEngine>(GEngine);
        USceneComponent* SelectedComponent = Engine ? Engine->GetSelectedComponent() : nullptr;
        AActor* SelectedActor = Engine ? Engine->GetSelectedActor() : nullptr;
        USceneComponent* TargetComponent = nullptr;
        if (SelectedComponent != nullptr) TargetComponent = SelectedComponent;
        else if (SelectedActor != nullptr) TargetComponent = SelectedActor->GetRootComponent();
        const bool bIsSelected = (TargetComponent == Comp);
        ...
        // (선택 사항) 스켈레탈 메시 바운딩 박스 그리기
        if (Viewport->GetShowFlag() & static_cast<uint64>(EEngineShowFlags::SF_AABB))
        {
            // 스켈레탈 메시는 바운딩 박스 계산 방식이 다를 수 있음.
            // 여기서는 컴포넌트의 바운딩 박스를 사용한다고 가정.
            FEngineLoop::PrimitiveDrawBatch.AddAABBToBatch(Comp->GetBoundingBox(), Comp->GetWorldLocation(), WorldMatrix);
        }
    }
}
